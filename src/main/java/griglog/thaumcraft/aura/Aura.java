...
public class Aura {
    ...
    public static AuraChunk getAuraChunk(Chunk chunk){
        return chunk.getCapability(Provider.AURA_CAP).resolve().map(value -> value.ac).orElse(null);
    }
    ...
    public static class Provider implements ICapabilitySerializable<INBT> {
        ...
        public INBT serializeNBT() {
            return AURA_CAP.getStorage().writeNBT(AURA_CAP, this.instance.resolve().get(), null);
        }

        @Override
        public void deserializeNBT(INBT nbt) {
            AURA_CAP.getStorage().readNBT(AURA_CAP, this.instance.resolve().get(), null, nbt);
        }
    }
